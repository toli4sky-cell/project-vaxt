...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=str,
        default="",
        help="Path to Excel file used as source schedule",
    )
...
def _normalize_excel_code(value: object) -> str:
    if value is None:
        return ""
...
def _excel_code_to_timesheet(code_raw: str, codes: dict[str, TimesheetCode]) -> tuple[TimesheetCode | None, str | None, int | None]:
    """
    Возвращает:
      (main_code, top_display_text, total_hours)
    Правила:
      - верхняя строка: К, С, РВД и служебные пометки
      - нижняя строка: В, ОТ, Д, Б, 4, 5, 6 и др.
    """
    code = _normalize_excel_code(code_raw)
    if not code:
        return None, None, None

    if code in {"К", "С", "РВД"}:
        main = codes.get(code)
        return main, code, None

    if code in {"В", "ОТ", "Д", "Б", "НВ", "4", "5", "6"}:
        main = codes.get(code)
        return main, None, None

    # прочие текстовые метки оставляем в верхней строке
...
def _cell_int(value: object) -> int | None:
    ...
    if n == 0:
        return None
    return int(n) if n.is_integer() else round(n)


def _find_sheet(workbook) -> str:
    preferred = ["СВОД", "ÑÂÎÄ"]
...
def import_schedule_from_excel(
    ...
    *,
    ...
    """
    Импортирует график из Excel в schedule_plan_entries.
    Текущая версия — каркас:
    - создаёт generation
    - читает лист
    - строит карту сотрудников по табельному номеру
    - ожидает табличную структуру как в твоём Excel
    """
    path = Path(xlsx_path)
    if not path.exists():
    ...
    gen = ScheduleGeneration(
        year=year,
        department_id=None,
        generation_type=GenerationType.ACTIVE,
        based_on_generation_id=None,
        started_by_id=None,
        started_at=now,
        completed_at=now,
        status=GenerationStatus.COMPLETED,
        notes=f"{DEMO_MARKER} imported from excel: {path.name}",
    )
    session.add(gen)
    session.flush()

    employees_by_tab = {
        str(emp.personnel_number).strip(): emp
        for emp in employees
        if emp.personnel_number
    }

    # -------------------------------------------------------
    # ВАЖНО:
    # Ниже допущение по структуре:
    # - строка с данными сотрудника
    # - следующая строка/строки содержат верхние/нижние коды и часы
    # Это место Cursor должен адаптировать под реальный файл.
    # -------------------------------------------------------

    max_col = ws.max_column
    max_row = ws.max_row

    # Пример: дни начинаются с колонки H (= 8)
    day_col_start = 8
    ...
    row = 1
    while row <= max_row:
        tab_value = ws.cell(row=row, column=3).value  # колонка C: табельный
        fio_value = ws.cell(row=row, column=2).value  # колонка B: ФИО

        tab_number = str(tab_value).strip() if tab_value is not None else ""
        emp = employees_by_tab.get(tab_number)

        if emp is None:
            row += 1
            continue

        # Предполагаем 3 строки на сотрудника:
        # row     -> верхняя строка (пометки)
        # row + 1 -> нижняя строка (коды)
        # row + 2 -> часы
        marks_row = row
        codes_row = row + 1
        hours_row = row + 2

        for col in range(day_col_start, max_col + 1):
            day_num_raw = ws.cell(row=2, column=col).value
            day_num = _cell_int(day_num_raw)
            if not day_num:
                continue

            try:
                work_date = date(year, EXCEL_V2_MONTH if 'EXCEL_V2_MONTH' in globals() else 3, day_num)
            except Exception:
                continue

            mark_val = ws.cell(row=marks_row, column=col).value
            code_val = ws.cell(row=codes_row, column=col).value
            hours_val = ws.cell(row=hours_row, column=col).value

            main_code = None
            top_display_text = None

            mark_code_obj, mark_top_text, _ = _excel_code_to_timesheet(str(mark_val or ""), codes)
            code_code_obj, code_top_text, _ = _excel_code_to_timesheet(str(code_val or ""), codes)

            # Приоритет нижней строки как основной код
            if code_code_obj is not None:
                main_code = code_code_obj
            elif mark_code_obj is not None:
                main_code = mark_code_obj

            if mark_top_text:
                top_display_text = mark_top_text
            elif code_top_text and main_code is None:
                top_display_text = code_top_text

            total_hours_val = _cell_int(hours_val)
            total_hours = Decimal(str(total_hours_val)) if total_hours_val is not None else Decimal("0")

            if main_code is None and not top_display_text and total_hours == Decimal("0"):
                continue
            ...
            session.add(
                SchedulePlanEntry(
                    ...
                    work_date=work_date,
                    template_id=None,
                    code_id=main_code.id if main_code else codes["Д"].id,
                    overlay_code_id=None,
                    total_hours=total_hours,
                    day_hours=total_hours,
                    night_hours=Decimal("0"),
                    top_display_text=top_display_text,
                    bottom_display_text=None,
                    style_type=None,
                    derived_presence=cell_counts_as_on_shift(main_code, total_hours) if main_code else False,
                )
            )
            created += 1
    ...
    session.flush()
    print(f"Imported plan entries from Excel: {created}")
    return gen